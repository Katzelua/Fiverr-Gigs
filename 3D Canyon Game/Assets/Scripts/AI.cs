    ...
    private void FixedUpdate()
    {
        ...
        switch (MovementStyle) {
            ...
            case PathMovementStyle.Continuous:
                var ab = new Vector3(_points[_currentTargetIdx].position.x, transform.position.y ,_points[_currentTargetIdx].position.z);
                transform.position = Vector3.MoveTowards(transform.position, ab, MovementSpeed * Time.deltaTime);
                transform.LookAt(ab);
                break;
